...
void Boids::initSimulation(int N) {
  ...
  cudaMalloc((void**)&dev_posAf, N * sizeof(glm::vec3));
  checkCUDAErrorWithLine("cudaMalloc dev_posAfterShuffle failed!");

  cudaMalloc((void**)&dev_velAf, N * sizeof(glm::vec3));
...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3 *pos, const glm::vec3 *vel) {
  // Rule 1: boids fly towards their local perceived center of mass, which excludes themselves
	...
	glm::vec3 rule1(0, 0, 0);
	glm::vec3 rule2(0, 0, 0);
	glm::vec3 rule3(0, 0, 0);
	glm::vec3 vc(0, 0, 0);
	int N1 = 0, N2 = 0, N3 = 0;
	...
	for (int i = 0; i < N; i++)
	{
		...
		{
			if (glm::length(pos[i] - pos[iSelf]) < 100)
			{
				c -= (pos[i] - pos[iSelf]);
			}
		}
...
__global__ void kernUpdateVelocityBruteForce(int N, glm::vec3 *pos,
  ...
  // Record the new velocity into vel2. Question: why NOT vel1?
	...
	//Why not vel1? because we need the former velocity to compute new velocity in rule3.
}
...
__global__ void kernComputeIndices(int N, int gridResolution,
  ...
  glm::vec3 *pos, int *indices, int *gridIndices) {
	...
	if (id < N)
	{
		int x = (pos[id].x - gridMin.x)*inverseCellWidth;
		int y = (pos[id].y - gridMin.y)*inverseCellWidth;
		int z = (pos[id].z - gridMin.z)*inverseCellWidth;
		gridIndices[id] = gridIndex3Dto1D(x, y, z, gridResolution);
		indices[id] = id;
	}
	else
	{
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  // - Clamp the speed change before putting the new speed in vel2
	...
	float x = (pos[particleIndex].x - gridMin.x) * inverseCellWidth;
	float y = (pos[particleIndex].y - gridMin.y) * inverseCellWidth;
	float z = (pos[particleIndex].z - gridMin.z) * inverseCellWidth;

	int dx = (x - (int)x) > 0.5f ? 1 : -1;
	int dy = (y - (int)y) > 0.5f ? 1 : -1;
	int dz = (x - (int)z) > 0.5f ? 1 : -1;

	int neighborsIndex[8];
	neighborsIndex[0] = gridIndex3Dto1D(x, y, z, gridResolution);
	neighborsIndex[1] = gridIndex3Dto1D(x + dx, y, z, gridResolution);
	neighborsIndex[2] = gridIndex3Dto1D(x + dx, y + dy, z, gridResolution);
	neighborsIndex[3] = gridIndex3Dto1D(x + dx, y, z + dz, gridResolution);
	neighborsIndex[4] = gridIndex3Dto1D(x + dx, y + dy, z + dz, gridResolution);
	neighborsIndex[5] = gridIndex3Dto1D(x, y + dy, z, gridResolution);
	neighborsIndex[6] = gridIndex3Dto1D(x, y + dy, z + dz, gridResolution);
	neighborsIndex[7] = gridIndex3Dto1D(x, y, z + dz, gridResolution);
	...
	for (int i = 0; i < 8; i++)
	{
		if (neighborsIndex[i] < 0 || neighborsIndex[i] >= gridCount)
		{
			continue;
		}
		if (gridCellStartIndices[neighborsIndex[i]] != -1 && gridCellEndIndices[neighborsIndex[i]] != -1)
		{
			for (int j = gridCellStartIndices[neighborsIndex[i]]; j <= gridCellEndIndices[neighborsIndex[i]]; j++)
			{
				int k = particleArrayIndices[j];
				//if the particle is not itself ***see the rule definition in 1.2
				if (particleIndex != k)
				{
					float distance = glm::length(pos[k] - pos[particleIndex]);
	...
	{
		perceived_center /= r1;
		v1 = (perceived_center - pos[particleIndex])* rule1Scale;
	}
	...
	if (r3 > 0)
	{
		perceived_velocity /= r3;
		v3 = perceived_velocity * rule3Scale;
	}

	
	glm::vec3 finVel = vel1[particleIndex] + v1 +v2 +v3;



	//clamp
	if (glm::length(finVel) > maxSpeed)
	{
		finVel = glm::normalize(finVel) * maxSpeed;
	}

	vel2[particleIndex] = finVel;

}


__global__ void kernRearrange(
	int N, int* particleArrayIndices, glm::vec3* pos, glm::vec3* posAf,
	glm::vec3* vel, glm::vec3* velAf)
{
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;
	if (index >= N) {
		return;
	}
	posAf[index] = pos[particleArrayIndices[index]];
	velAf[index] = vel[particleArrayIndices[index]];
...
__global__ void kernUpdateVelNeighborSearchCoherent(
  ...
  // - Clamp the speed change before putting the new speed in vel2
	...
	glm::vec3 finVel = vel1[index] + v1 + v2 + v3;
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  // - Ping-pong buffers as needed. THIS MAY BE DIFFERENT FROM BEFORE.
	...
	kernRearrange<< <fullBlocksPerGrid, blockSize >> >(numObjects,
		dev_particleArrayIndices, dev_pos, dev_posAf, dev_vel1, dev_velAf);
	...
	kernUpdateVelNeighborSearchCoherent << <fullBlocksPerGrid, blockSize >> >(numObjects, gridSideCount, gridMinimum, gridInverseCellWidth, gridCellWidth,
		dev_gridCellStartIndices, dev_gridCellEndIndices, dev_posAf, dev_velAf, dev_vel2);

	std::swap(dev_pos, dev_posAf);
